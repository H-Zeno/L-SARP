...
class RobotPlanner:
    ...
    async def create_task_plan(self, additional_message: Annotated[str, "Additional message to add to the task generation prompt"] = "") -> None:
        if self.json_format_chat_history is None:
            self.json_format_chat_history = ChatHistory()
            
        plan_generation_prompt = create_task_planning_prompt_template.format(goal=self.goal, scene_graph=scene_graph_to_json(robot_state.scene_graph), robot_position=frame_transformer.get_current_body_position_in_frame(robot_state.frame_name))

        logger.info("========================================")
        logger.info(f"Plan generation prompt: {plan_generation_prompt}")
        logger.info("========================================")
        plan_response, self.json_format_chat_history = await invoke_agent(self.task_planner_agent, additional_message + plan_generation_prompt, chat_history=self.json_format_chat_history)
    ...
    async def update_task_plan(self, issue_description: Annotated[str, "Detailed description of the current explanation and what went different to the original plan"]) -> None:
        ...
        updated_plan_response, self.json_format_chat_history = await invoke_agent(self.task_planner_agent, update_plan_prompt, chat_history=self.json_format_chat_history)
